  ...
  <div class="space-y-3">
    <template v-for="it in items" :key="it.id">
      ...
      >
        ...
        <div class="whitespace-pre-wrap break-words text-zinc-100 text-[13.5px] leading-relaxed">{{ it.content }}</div>
      </div>

      <!-- Assistant -->
      <div
        v-else-if="it.type === 'assistant'"
        class="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3.5"
      >
        <div class="flex items-center justify-between mb-1.5">
          <div class="flex items-center gap-1.5 text-[11px] font-medium text-zinc-400 uppercase tracking-wider">
      ...
      <details
        ...
        class="rounded-lg border border-zinc-800/60 bg-zinc-900/20 px-3 py-2 text-xs"
      >
        <summary class="cursor-pointer text-zinc-500 hover:text-zinc-300 select-none flex items-center gap-2">
          <span class="text-[10px] uppercase tracking-wider">Reasoning</span>
          <span class="text-zinc-600">·</span>
          <span>{{ fmtTs(it.timestamp) }}</span>
        </summary>
        <pre class="mt-2 whitespace-pre-wrap break-words text-zinc-400 text-xs leading-relaxed">{{ it.content }}</pre>
      </details>
      ...
      >
        ...
        <pre class="text-[11.5px] text-zinc-300 whitespace-pre-wrap break-words max-h-44 overflow-y-auto font-mono">{{ truncate(it.content, 4000) }}</pre>
      </div>

      <!-- Tool result -->
      <details
        v-else-if="it.type === 'tool_result' && showToolResults"
        class="rounded-lg border border-zinc-800/60 bg-zinc-900/20 px-3 py-2"
      >
        <summary class="cursor-pointer text-zinc-500 hover:text-zinc-300 select-none flex items-center justify-between text-xs">
        ...
        <pre class="mt-2 text-[11.5px] text-zinc-400 whitespace-pre-wrap break-words max-h-60 overflow-y-auto font-mono">{{ truncate(it.content, 4000) }}</pre>
      </details>
